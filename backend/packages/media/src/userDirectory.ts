...
const userCache = new Map<
  string,
  { expiresAt: number; profile: BasicUserProfile }
>();

export async function resolveCoreUserProfiles(
  userIds: string[],
  ...
  if (missingIds.length) {
    const fetched = await Promise.all(
      missingIds.map((userId) => fetchCoreUserProfile(userId)),
    );
...
async function fetchCoreUserProfile(userId: string): Promise<BasicUserProfile> {
  ...
  try {
    const response = await fetch(
      `${baseUrl}/v1/users/${encodeURIComponent(userId)}/profile`,
    );
