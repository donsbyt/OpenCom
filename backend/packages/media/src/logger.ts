...
function toErrorContext(error: unknown): Ctx {
  if (!(error instanceof Error)) {
    return { errorMessage: String(error || "UNKNOWN_ERROR") };
  }
  const base: Ctx = {
...
function write(level: Level, scope: string, message: string, context?: Ctx) {
  ...
  if (env.LOG_TO_FILE) appendLogFileLine(line);
...
function appendLogFileLine(line: string) {
  try {
    ...
    const filePath = path.join(env.LOG_DIR, `media-${day}.log`);
    fs.appendFileSync(filePath, `${line}\n`, "utf8");
  } catch {
    // Avoid crashing logging if the filesystem is unavailable.
  }
}
