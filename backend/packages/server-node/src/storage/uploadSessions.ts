...
import { buildPath, ensureDir, randomId } from "./fsStore.js";
...
export async function appendUploadChunk(
  ...
  const writtenBytes = await streamChunkToFile(
    buildPath(rootDir, [session.tempObjectKey]),
    readable,
    currentSize,
...
export function finalizeUploadSession(rootDir: string, uploadId: string): UploadSession | null {
  ...
  const tempPath = buildPath(rootDir, [session.tempObjectKey]);
  const finalPath = buildPath(rootDir, [session.finalObjectKey]);
...
export function destroyUploadSession(rootDir: string, uploadId: string) {
  ...
  const tempPath = buildPath(rootDir, [path.join(SESSION_DIR, `${uploadId}.part`)]);
...
function ensureSessionDir(rootDir: string) {
  ensureDir(rootDir);
  const sessionDir = buildPath(rootDir, [SESSION_DIR]);
...
function getUploadSize(rootDir: string, tempObjectKey: string) {
  try {
    return Number(fs.statSync(buildPath(rootDir, [tempObjectKey])).size || 0);
  } catch {
    return 0;
  }
}
